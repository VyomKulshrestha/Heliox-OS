...
SSH_ACCEPTED_PATTERN = re.compile(
    r"Accepted\s+(?:password|publickey)\s+for\s+(\S+)\s+from\s+(\S+)\s+port\s+(\d+)\s+ssh2", re.IGNORECASE
)

# Sudo regexes
SUDO_PATTERN = re.compile(r"(\S+)\s+:\s+TTY=\S+\s+;\s+PWD=\S+\s+;\s+USER=(\S+)\s+;\s+COMMAND=(.*)", re.IGNORECASE)
