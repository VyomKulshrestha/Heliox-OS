...
def detect_spikes(
    events: list[dict[str, Any]], window_minutes: int = 10, threshold_count: int = 5
) -> list[dict[str, Any]]:
    ...
    restart_events = [e for e in events if "restart" in e["message"].lower() or "started" in e["message"].lower()]
    ...
    for proc, evs in process_restarts.items():
        ...
        for i in range(len(evs_sorted)):
            ...
            for j in range(i - 1, -1, -1):
                try:
                    dt_start_str = (
                        evs_sorted[j]["timestamp"].replace("Z", "").split(".")[0].split("+")[0].replace("T", " ")
                    )
    ...
    failed_logins = [
        e for e in events if "login_failed" in e.get("event_type", "") or "ssh_login_failed" in e.get("event_type", "")
    ]
    ...
    error_events = [
        e for e in events if any(err in e["message"].lower() for err in ["error", "fail", "critical", "panic", "crash"])
    ]
        ...
        anomalies.append(
            {
                ...
                "details": {"errors_count": len(error_events)},
