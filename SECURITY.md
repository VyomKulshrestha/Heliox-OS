...
**Free and fully local, by design.** Kyutai's Pocket TTS code is MIT/Apache-2.0 and the model weights are CC-BY-4.0 — no API key, no per-request cost, no cloud inference of any kind. The only network activity is a one-time ~236MB model download from Hugging Face the first time a user who has actually installed the optional `pocket-tts` package calls `speak()` — after that, everything runs offline on the user's own CPU.

**Safe by default even when not installed.** `pocket-tts` is deliberately **not** a hard dependency — it's an optional extra (`pilot-daemon[voice]`, see `pyproject.toml`), the same treatment `openai-whisper` already gets in this codebase (never installed in the CI-locked `requirements-lock.txt`, always behind an `except ImportError` fallback). `_speak_impl`'s Pocket TTS branch catches `ImportError` and any other synthesis/playback failure and falls straight through to the pre-existing OS-native dispatch — so a machine that hasn't opted into the extra behaves exactly as it did before this pass, with zero network calls ever attempted.

**Known scope limits, stated plainly.** Uses Pocket TTS's one-shot `generate_audio()` API, not its chunked/streaming mode — simpler and consistent with `speak()`'s existing "one atomic, cancellable operation" model, at the cost of the ~200ms first-chunk-latency improvement streaming would give. Only the default English model/voice set is wired up (`tts_voice`, a handful of built-in presets) — the additional languages Pocket TTS has since added are not surfaced in Settings. The cognitive-load-based speech-rate modulation `_speak_impl` already applies to the OS-native paths does not carry over to Pocket TTS, which has no equivalent integer rate knob. Not verified end-to-end against the real model in this pass — `pocket-tts` is never actually installed in this dev/CI environment, so automated tests cover the fallback/mocked paths only (fake-module injection mirroring `test_voice_whisper_cache.py`), not real model download, real CPU inference, or real audio output.
